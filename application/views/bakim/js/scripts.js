...
$(function() {
    ...
    $(window).resize(function(){
      if($(window).width() > 768) {
        var stickyMaxHeight = $('.header').height();
      } else {
        var stickyMaxHeight = $('.mobile-header').height();
      }

        $(window).scroll(function(){
          if($(this).scrollTop() > stickyMaxHeight) {
  ...
  var googleMap = function() {
    var obj = "#gmap";
		var image = 'http://academic.webtorn.com/assets/frontend/default/images/marker.png';
		var zoomLevel = 17;

    var mapOptions = {
			zoom: zoomLevel,
			disableDefaultUI: true,
			center: new google.maps.LatLng(41.0157103,28.9746705),
      scrollwheel: false,
    ...
    var coordinates = $(obj).attr("data-coordinates");
    $.each(JSON.parse(coordinates), function(title, data){

      var coordinate = data.coordinate.split(',');
      var lat = coordinate[0];
      var lng = coordinate[1];
      var contentString = '<div class="gmap-title">' + title + '</div>';
      ...
      google.maps.event.addListenerOnce(map, 'idle', function(){
        $('.gmap-title').parent().parent().parent().parent().find('> div:first').remove();
        $('.gmap-title').parent().parent().parent().parent().find('> div:last').remove();
        $('.gmap-title').closest('.gm-style-iw').css('top', '30px');
      });
  };
