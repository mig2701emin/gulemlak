...
$baslik=(!empty($baslik_p)?$baslik_p:$proje[baslik]);																								
$icerik_p=guvenlik($_POST['icerik'],1);	
$icerik=(!empty($icerik_p)?$icerik_p:base64_decode($proje[aciklama]));	
$kategori_p=guvenlik($_POST['kategori']);	
$kategori=(!empty($kategori_p)?$kategori_p:$proje[kategori]);		
$kategori2_p=guvenlik($_POST['kategori2']);	
$kategori2=(!empty($kategori2_p)?$kategori2_p:$proje[kategori2]);		
$firma_p=guvenlik($_POST['firma']);
$firma=(!empty($firma_p)?$firma_p:$proje[firma]);			
$sehir_p=guvenlik($_POST['sehir']);
$sehir=(!empty($sehir_p)?$sehir_p:$proje[sehir]);			
$ilce_p=guvenlik($_POST['ilce']);
$ilce=(!empty($ilce_p)?$ilce_p:$proje[ilce]);			
$m2araligi1_p=guvenlik($_POST['m2araligi1']);		
$m2araligi2_p=guvenlik($_POST['m2araligi2']);		
$m2araligi_p=$m2araligi1_p." m2 - ".$m2araligi2_p." m2";
$explodem2=explode(" - ",$proje[m2araligi]);
$m2araligi1=(!empty($m2araligi1_p)?$m2araligi1_p:str_replace(" m2","",$explodem2[0]));	
$m2araligi2=(!empty($m2araligi2_p)?$m2araligi2_p:str_replace(" m2","",$explodem2[1]));	
$m2araligi=(!empty($m2araligi1_p)?$m2araligi_p:$m2araligi1." - ".$m2araligi2);	
$projealani_p=guvenlik($_POST['projealani']);
$projealani=(!empty($projealani_p)?$projealani_p:$proje[projealani]);
$sehir=(!empty($sehir_p)?$sehir_p:$proje[sehir]);				
$emlaktipi_p=guvenlik($_POST['emlaktipi']);		
$emlaktipi=(!empty($emlaktipi_p)?$emlaktipi_p:$proje[emlaktipi]);		
$odasecenekleri1_p=guvenlik($_POST['odasecenekleri1']);		
$odasecenekleri2_p=guvenlik($_POST['odasecenekleri2']);			
$explodeoda=explode("+",$proje[odasecenekleri]);
$odasecenekleri1=(!empty($odasecenekleri1_p)?$odasecenekleri1_p:$explodeoda[0]);	
$odasecenekleri2=(!empty($odasecenekleri2_p)?$odasecenekleri2_p:$explodeoda[1]);	
$odasecenekleri_p=$odasecenekleri1."+".$odasecenekleri2;
$odasecenekleri=(!empty($odasecenekleri_p)?$odasecenekleri_p:$odasecenekleri1."+".$odasecenekleri2);	
$sehir=(!empty($sehir_p)?$sehir_p:$proje[sehir]);	
$teslimtarihi_p=guvenlik($_POST['teslimtarihi'],1);		
$teslimtarihi=(!empty($teslimtarihi_p)?$teslimtarihi_p:$proje[teslimtarihi]);	
$iletisim_p=guvenlik($_POST['iletisim']);	
$iletisim=(!empty($iletisim_p)?$iletisim_p:$proje[iletisim]);		
$map_p=guvenlik($_POST['map_Val']);
$map=(!empty($map_p)?$map_p:$proje[map]);	
$genelozellikler_p=guvenlik(implode(",",$_POST['genelozellikler']));
$genelozellikler=(!empty($genelozellikler_p)?$genelozellikler_p:$proje[genelozellikler]);	
$genelozellikler_check=explode(",",$genelozellikler);	
$durum_p=guvenlik($_POST['durum']);			
$durum=(!empty($durum_p)?$durum_p:$proje[durum]);	
$img=$_FILES['img'];
$url=seo($baslik);										
...
<script src="../js/edit_project_map.php?currentlatlong=<?=base64_encode($map);?>"></script>
<script>
...
if(count($DestinationFile_multi)!=0){
$resimSQL=",resimler='$resimler'";
}else{
$resimSQL="";
}
if($logo!=0){
...
process_mysql("update projeler set baslik='$baslik',seo_url='$url',aciklama='$icerik_b64',kategori='$kategori',kategori2='$kategori2',firma='$firma',sehir='$sehir',ilce='$ilce',m2araligi='$m2araligi',projealani='$projealani',emlaktipi='$emlaktipi',odasecenekleri='$odasecenekleri',teslimtarihi='$teslimtarihi',iletisim='$iletisim',map='$map',genelozellikler='$genelozellikler',durum='$durum'".$logoSQL.$resimSQL." where Id='".$Id."'","proje-yonetimi.html");
}
?>
			<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i> Proje Düzenle</h2>
						<div class="box-icon">
...
<fieldset>
<legend>Proje Düzenle : <?=$proje[baslik];?></legend>
							...
							<div class="control-group">
							  ...
							  <div class="controls">
								<input type="file" name="img" value="<?=$logo;?>">
								</div>
							</div>
							<div class="control-group">
