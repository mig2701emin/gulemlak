...
function delete_id(id)
{
     ...
     {
        window.location.href='borcsil.php?delete_id='+id;
     }
}
</script>
...
<?php
	...
	if(!empty($_POST["islem"])){//Tarih aralığına göre bilgi için
		$trh1=$_POST["trh1"];
		$trh2=$_POST["trh2"];
		$sqlt="select tutar from borc where tarih between '$trh1' and '$trh2'";
...
<center><form id="form1" name="form1" method="post" action="borcrapor.php">
<table width="1080" border="1" style="max-width:1080px; min-width:1079px; list-style-type:none; position:relative;  width: 1080px; background-color:#FFF0EC;">
  <tr>
  	<td align="center" valign="middle"><font color="#0000FF">Toplam<br />Borç</font></td>
    ...
    <td align="center" valign="middle"><font color="#0000FF">Tarih Aralığına Göre<br />
       Toplam Borç Öğren</font></td>
    <td align="center" valign="middle"><span id="sprytextfield1">
    <label for="trh1"></label>
    <input type="text" name="trh1" id="trh1" value="<?php if(!empty($_POST["islem"])){ echo $trh2; } ?>" /><br />
    ...
    <span class="textfieldRequiredMsg">Lütfen tarih giriniz.</span><span class="textfieldInvalidFormatMsg">Tarihi 2011-11-11 şeklinde giriniz.</span></span>
    </td><td align="center"><font color="#00FF00" size="+3"><?php if(!empty($_POST["islem"])){$toplamt=number_format($toplamt, 2, ',', '.'); echo $toplamt; } ?></font></td>
    <td align="center" valign="middle"><input type="submit" name="islem" id="islem" value="Hesapla" /></td>
  </tr>
</table></form></center>
  <p><H1 align="center" style="color:#00F;">BORÇLAR</H1></p>
</center></p>
...
<center>
  ...
  <tr>
    ...
    <td align="center" valign="middle"><font color="#0000FF">Borç Tarihi</font></td>
...
<?php
	$x=1;
	$sql="select *,DATE_FORMAT(tarih, '%d.%m.%Y') as trh from borc order by tarih desc";
	...
	while($deger=@mysql_fetch_array($sorgu)){
		...
		echo "$tutarc</td><td>".$deger["aciklama"]."</td><td><form name=g$x id=g$x action=borc.php method=post><input type=hidden name=id value='".$deger["id"]."'><input type=submit name=bt1 value=Düzelt></form></td><td><form name=f$x action='borcsil.php' method=post onsubmit='return false;'><input type=hidden name=barkod value='".$deger["id"]."'>"; ?>
...
<script type="text/javascript">
var sprytextfield1 = new Spry.Widget.ValidationTextField("sprytextfield1", "date", {format:"yyyy-mm-dd", hint:"2011-11-11", useCharacterMasking:true});
var sprytextfield2 = new Spry.Widget.ValidationTextField("sprytextfield2", "date", {format:"yyyy-mm-dd", hint:"2011-11-11", useCharacterMasking:true});
</script>
</body>
</html>
<?php
