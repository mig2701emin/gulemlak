...
function delete_id(id)
{
     ...
     {
        window.location.href='alacaksil.php?delete_id='+id;
     }
}
</script>
...
<?php
	...
	if(!empty($_POST["islem"])){//Tarih aralığına göre bilgi için
		$trh1=$_POST["trh1"];
		$trh2=$_POST["trh2"];
		$sqlt="select tutar from alacak where tarih between '$trh1' and '$trh2'";
...
<center><form id="form1" name="form1" method="post" action="alacakrapor.php">
<table width="1080" border="1" style="max-width:1080px; min-width:1079px; list-style-type:none; position:relative;  width: 1080px; background-color:#FFF0EC;">
  <tr>
  	<td align="center" valign="middle"><font color="#0000FF">Toplam<br />Alacak</font></td>
  ...
  <p><H1 align="center" style="color:#00F;">ALACAKLAR</H1></p>
</center></p>
 <center>
  ...
  <tr>
    ...
    <td align="center" valign="middle"><font color="#0000FF">Alacak Tarihi</font></td>
...
<?php
	$x=1;
	$sql="select *,DATE_FORMAT(tarih, '%d.%m.%Y') as trh from alacak order by tarih desc";
	...
	while($deger=@mysql_fetch_array($sorgu)){
		...
		echo "$tutarc</td><td>".$deger["aciklama"]."</td><td><form name=g$x id=g$x action=alacak.php method=post><input type=hidden name=id value='".$deger["id"]."'><input type=submit name=bt1 value=Düzelt></form></td><td><form name=f$x action='alacaksil.php' method=post onsubmit='return false;'><input type=hidden name=barkod value='".$deger["id"]."'>"; ?>
