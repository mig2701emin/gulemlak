...
function delete_id(id)
{
     ...
     {
        window.location.href='tahsilatsil.php?delete_id='+id;
     }
}
</script>
...
<body>
<?php
	$sqlc="select tutar from tahsilat";
	...
	if(!empty($_POST["islem"])){//Tarih aralığına göre bilgi için
		$trh1=$_POST["trh1"];
		$trh2=$_POST["trh2"];
		$sqlt="select tutar from tahsilat where tarih between '$trh1' and '$trh2'";
...
<center><form id="form1" name="form1" method="post" action="tahsilatrapor.php">
<table width="1080" border="1" style="max-width:1080px; min-width:1079px; list-style-type:none; position:relative;  width: 1080px; background-color:#FFF0EC;">
  <tr>
  	<td align="center" valign="middle"><font color="#0000FF">Toplam<br />Tahsilat</font></td>
  ...
  <p><H1 align="center" style="color:#00F;">YAPILAN TAHSİLATLAR</H1></p>
</center></p>
...
<center>
  ...
  <tr>
    ...
    <td align="center" valign="middle"><font color="#0000FF">Tahsilat Tarihi</font></td>
    ...
    <td align="center" valign="middle"><font color="#0000FF">Tahsilat Türü</font></td>
...
<?php
	$x=1;
	$sql="select *,DATE_FORMAT(tarih, '%d.%m.%Y') as trh from tahsilat order by tarih desc";
	...
	while($deger=@mysql_fetch_array($sorgu)){
		...
		echo "<td>".$deger["tahturu"]."</td><td align=right>";
		$tutarc=$deger["tutar"];
		$tutarc=number_format($tutarc, 2, ',', '.');
		echo "$tutarc</td><td>".$deger["aciklama"]."</td><td><form name=g$x id=g$x action=tahsilatyap.php method=post><input type=hidden name=id value='".$deger["id"]."'><input type=submit name=bt1 value=Düzelt></form></td><td><form name=f$x action='tahsilatsil.php' method=post onsubmit='return false;'><input type=hidden name=barkod value='".$deger["id"]."'>"; ?>
