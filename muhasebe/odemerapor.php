...
function delete_id(id)
{
     ...
     {
        window.location.href='odemesil.php?delete_id='+id;
     }
}
</script>
...
<?php
	...
	if(!empty($_POST["islem"])){//Tarih aralığına göre bilgi için
		$trh1=$_POST["trh1"];
		$trh2=$_POST["trh2"];
		$sqlt="select tutar from odeme where tarih between '$trh1' and '$trh2'";
...
?>
<p><center><a href="anamenu.php"><img src="baslik.png" alt="" name="" width="253" height="50" align="middle" /></a></center></p>
<center><form id="form1" name="form1" method="post" action="odemerapor.php">
<table width="1080" border="1" style="max-width:1080px; min-width:1079px; list-style-type:none; position:relative;  width: 1080px; background-color:#FFF0EC;">
  <tr>
  	<td align="center" valign="middle"><font color="#0000FF">Toplam<br />Ödeme</font></td>
    <td align="center" valign="middle"><b><?php  $toplam=number_format($toplam, 2, ',', '.'); echo $toplam; ?></b></td>
    <td align="center" valign="middle"><font color="#0000FF">Tarih Aralığına Göre<br />
       Toplam Ödeme Öğren</font></td>
    <td align="center" valign="middle">
      <label for="trh1"></label>
      <span id="sprytextfield3">
      <label for="trh3"></label>
      <input type="text" name="trh1" id="trh3" value="<?php if(!empty($_POST["islem"])){ echo $trh1; } ?>" /><BR />
      <span class="textfieldRequiredMsg">Lütfen tarih giriniz.</span><span class="textfieldInvalidFormatMsg">Tarihi 2011-11-11 şeklinde giriniz.</span></span>
    </td>
    <td align="center" valign="middle"><font color="#0000FF">
    <label for="trh2"></label>
    <span id="sprytextfield4">
    <label for="trh4"></label>
    <input type="text" name="trh2" id="trh4" value="<?php if(!empty($_POST["islem"])){ echo $trh2; } ?>" /><br />
    <span class="textfieldRequiredMsg">Lütfen tarih giriniz.</span><span class="textfieldInvalidFormatMsg">Tarihi 2011-11-11 şeklinde giriniz.</span></span>
    </font></td><td align="center"><font color="#00FF00" size="+3"><?php if(!empty($_POST["islem"])){ $toplamt=number_format($toplamt, 2, ',', '.'); echo $toplamt; } ?></font></td>
    <td align="center" valign="middle"><input type="submit" name="islem" id="islem" value="Hesapla" /></td>
  </tr>
</table></form></center>
  <p><H1 align="center" style="color:#00F;">ÖDEMELER</H1></p>
</center></p>
 ...
 <center>
  ...
  <tr>
    ...
    <td align="center" valign="middle"><font color="#0000FF">Ödeme Tarihi</font></td>
    <td align="center" valign="middle"><font color="#0000FF">Müşteri Ad Soyad</font></td>
    <td align="center" valign="middle"><font color="#0000FF">Müşteri Telefonu</font></td>
    <td align="center" valign="middle"><font color="#0000FF">Ödeme Türü</font></td>
...
<?php
	$x=1;
	$sql="select *,DATE_FORMAT(tarih, '%d.%m.%Y') as trh from odeme order by tarih desc";
	$sorgu=@mysql_query($sql);
	while($deger=@mysql_fetch_array($sorgu)){
		echo "<tr>";
		echo "<td>$x</td><td>".$deger["trh"]."</td><td>".$deger["kisi"]."</td>";
		$mid=$deger["mid"];
		$sqls="select * from musteri where id=$mid";
		$sorgus=@mysql_query($sqls);
		$degers=@mysql_fetch_array($sorgus);
		echo "<td>".$degers["tel"]."</td>";
		echo "<td>".$deger["odemeturu"]."</td><td align=right>";
		$tutarc=$deger["tutar"];
		$tutarc=number_format($tutarc, 2, ',', '.');
		echo "$tutarc</td><td>".$deger["aciklama"]."</td><td><form name=g$x id=g$x action=odemeyap.php method=post><input type=hidden name=id value='".$deger["id"]."'><input type=submit name=bt1 value=Düzelt></form></td><td><form name=f$x action='odemesil.php' method=post onsubmit='return false;'><input type=hidden name=barkod value='".$deger["id"]."'>"; ?>
...
<script type="text/javascript">
var sprytextfield3 = new Spry.Widget.ValidationTextField("sprytextfield3", "date", {format:"yyyy-mm-dd", useCharacterMasking:true, hint:"2011-11-11"});
var sprytextfield4 = new Spry.Widget.ValidationTextField("sprytextfield4", "date", {format:"yyyy-mm-dd", useCharacterMasking:true, hint:"2011-11-11"});
</script>
</body>
</html>
<?php
