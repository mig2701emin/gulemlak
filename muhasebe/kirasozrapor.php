...
function delete_id(id)
{
     ...
     {
        window.location.href='kirasozraporsil.php?delete_id='+id;
     }
}
</script>
...
<p><center>
 ...
 <center>
  ...
  <tr>
    ...
    <td align="center" valign="middle"><font color="#0000FF">Kira Bedeli</font></td>
    <td align="center" valign="middle"><font color="#0000FF">Depozito</font></td>
    <td align="center" valign="middle"><font color="#0000FF">Kira Başlangıcı</font></td>
    <td align="center" valign="middle"><font color="#0000FF">Kiraya Veren</font></td>
    <td align="center" valign="middle"><font color="#0000FF">Tel</font></td>
    <td align="center" valign="middle"><font color="#0000FF">Kiralayan</font></td>
    <td align="center" valign="middle"><font color="#0000FF">Tel</font></td>
...
<?php
	$x=1;
	$sql="select *,DATE_FORMAT(tarih, '%d.%m.%Y') as trh from kirasozlesme order by tarih desc";
	$sorgu=@mysql_query($sql);
	while($deger=@mysql_fetch_array($sorgu)){
		echo "<tr>";
		echo "<td>$x</td><td>".$deger["trh"]."</td>";
		echo "<td>".$deger["cinsi"]."</td><td align=right>";
		$aylik1=$deger["aylik1"]; $aylik1=number_format($aylik1, 2, ',', '.');
		echo "$aylik1</td><td align=right>";
		$depo=$deger["depozito"]; $depo=number_format($depo, 2, ',', '.');
		echo "$depo</td><td>".$deger["basi"]."</td><td>".$deger["satad"]."</td><td>".$deger["sattel"]."</td><td>".$deger["alad"]."</td><td>".$deger["altel"]."</td><td><form name=g$x id=g$x action=kirasozduzenle.php method=post><input type=hidden name=id value='".$deger["id"]."'><input type=submit name=bt5 value=Düzelt></form></td><td><form name=r$x id=r$x action=kirasozyazdir.php method=post><input type=hidden name=id value='".$deger["id"]."'><input type=submit name=bt15 value=Yazdır></form></td><td><form name=f$x action='kirasozraporsil.php' method=post onsubmit='return false;'><input type=hidden name=barkod value='".$deger["id"]."'>"; ?>
